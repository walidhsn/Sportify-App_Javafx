 ...
 * @author WALID
 */
public class UPDATE_sponsorController implements Initializable {
    ...
    @FXML
    private void update_sponsor(ActionEvent event) {
        Sponsor verif;
        ...
        if (name.isEmpty() && name.length() < 3) {
            ...
            verif = ss.find_sponsor_update(this.sponsor.getId(), name, email, tel);
            if (verif == null) {
                this.sponsor.setNomSponsor(name);
                this.sponsor.setEmailSponsor(email);
                this.sponsor.setTelSponsor(Integer.parseInt(tel));
                ss.updateEntity(this.sponsor);

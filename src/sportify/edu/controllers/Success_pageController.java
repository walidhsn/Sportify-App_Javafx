...
public class Success_pageController implements Initializable {
    ...
    public void setData(Reservation r) {
        ...
        send_sms_to_Client(Integer.parseInt(user.getPhone().trim())); // this will be replaced with the client number 
    }
    ...
    private void getReceiptPdf(ActionEvent event) {
        ...
        String html = "<!DOCTYPE html>\n"
                ...
                + "        <p><strong>Telephone:</strong> " + user.getPhone() + "</p>\n"
                + "    </div>\n"
                + "    <table class=\"items\">\n"
                + "        <thead>\n"
                + "            <tr>\n"
        ...
        for (Equipment item : list_eq) {
            html+=" <tr>"
                   +" <td colspan=\"3\" align=\"right\">Equipments:</td>"+
                  " <td>"+item.getName()+"</td>"+
                    " <td>"+item.getPrice()+" Dt"+"</td>"
                    + " </tr>";
        }
        html+="<tr>"+"                <td colspan=\"3\" align=\"right\">Total:</td>\n"

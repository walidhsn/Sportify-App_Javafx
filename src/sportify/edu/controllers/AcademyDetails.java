...
public class AcademyDetails implements Initializable {
    ...
    public void handleEditButtonClick(ActionEvent event) throws IOException, SQLException {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.AcademyEdit));
    ...
    @FXML
    private void Members(javafx.event.ActionEvent event) throws IOException, SQLException {
        String idText = txtId.getText();
        if (idText.isEmpty()) {
            // Display an error message to the user
            Alert alert = new Alert(AlertType.ERROR, "Please enter an ID");
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource(Constants.Members));
        Parent root = loader.load();

        // Get the controller of the AcademyEdit.fxml file
        Members controller = loader.getController();

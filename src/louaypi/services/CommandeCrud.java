...
public class CommandeCrud {
 ...
 /*public  Document createPDF(Commande s) throws DocumentException, FileNotFoundException {
        Document document = new Document();
        PdfWriter.getInstance(document, new FileOutputStream(s.getFirstname()+".pdf"));
        Paragraph title = new Paragraph("pdf de commande ");
        document.open();
        document.add(title);
        PdfPTable table = new PdfPTable(2);
        ...
        table.addCell("Tel :");
        table.addCell(s.getTel());

        table.addCell("Total Price:");
        table.addCell(Float.toString(s.getTotal()));
